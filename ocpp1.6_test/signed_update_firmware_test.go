...
func (suite *OcppV16TestSuite) TestSignedUpdateFirmwareResponseValidation() {
	...
	var responseTable = []GenericTestEntry{
		...
		{securefirmware.SignedUpdateFirmwareResponse{Status: securefirmware.UpdateFirmwareStatusAccepted}, false},
	}
...
func (suite *OcppV16TestSuite) TestSignedUpdateFirmwareE2EMocked() {
	...
	requestJson := fmt.Sprintf(`[2,"%v","%v",{"retries":%v,"retryInterval":%v,"requestId":%v,"securefirmware":{"location":"%v","retrieveDateTime":"%v","installDateTime":"%v","signingCertificate":"%v","signature":"%v"}}]`,
		messageId, securefirmware.SignedFirmwareStatusNotificationFeatureName, *retries, *retryInterval, requestID, fw.Location, fw.RetrieveDateTime.FormatTimestamp(), fw.InstallDateTime.FormatTimestamp(), fw.SigningCertificate, fw.Signature)
	...
	handler := &MockChargePointSecureFirmwareUpdateHandler{}
	handler.On("OnUpdateFirmware", mock.Anything).Return(updateFirmwareResponse, nil).Run(func(args mock.Arguments) {
		request := args.Get(0).(*securefirmware.SignedUpdateFirmwareRequest)
